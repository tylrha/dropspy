...
import { generateNewStoreDate, getStoreDateFromDatabase, IStoreDateProduct, saveStoreDateInDatabase } from './models/StoreDate';
...
export default async function addNewStoreSalesToDatabase(alihunterSalesArr: IAlihunterSale[], storeIndex: number, spoBotInstance: Spybot): Promise<void> {
  ...
  for (let x = 0; x < salesGroupedByProductsArr.length; x++) {
    ...
    LOGGER(`${saleIndex} - Adicionando o produto: ${saleObject.productName} aos bancos de dados | ${currentTrackedSales}`, { from: 'SPYBOT', pid: true });
...
async function addSaleToDatabase(saleObject: ISaleProduct, allLabels: ILabelMongo[]) {
  ...
  const newStoreDateProducts: IStoreDateProduct[] = storeDatesObjectInDatabase.products;
  let productIndex = newStoreDateProducts.findIndex((product) => product.productLink === saleObject.productLink);
  ...
  // STORE -----------------------------------------------
  const storeObjectInDatabase: IStoreMongo = await getStoreInDatabase(saleObject.storeLink);
